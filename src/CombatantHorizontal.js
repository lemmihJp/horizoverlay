  ...
  render() {
    ...
    if (config.color === 'byRole') {
      for (const role in jobRoles) {
        if (jobRoles[role].indexOf(data.Job.toLowerCase()) >= 0) jobStyleClass = ` job-${role}`;
        if (data.Job === '') {
    ...
    // Damage Percent
    damageWidth = `${parseInt((data.damage / this.props.encounterDamage) * 100, 10)}%`;
    ...
    return (
      <div
        className={`row ${data.Job}${jobStyleClass}${isSelf ? ' self' : ''}${
          isSelf && config.showHighlightSelf ? ' highlightSelf' : ''
        }`}
        style={{ order }}
      >
        <div className="name">
